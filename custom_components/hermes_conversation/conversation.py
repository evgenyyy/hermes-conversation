...
from .response_control import (
    ConversationResponseControl,
    apply_response_control,
    extract_response_control,
)
...
class WebhookConversationEntity(
    ...
    async def _async_handle_message(
        ...
        response_control = await self._async_handle_chat_log(user_input, chat_log)

        result = conversation.async_get_result_from_chat_log(user_input, chat_log)
        return cast(
            conversation.ConversationResult,
            apply_response_control(result, response_control),
        )
    ...
    async def _async_handle_chat_log(
        ...
        response_control = ConversationResponseControl()

        for _iteration in range(MAX_TOOL_ITERATIONS):
            if self._streaming_enabled:
                async for _ in chat_log.async_add_delta_content_stream(
                    self.entity_id,
                    self._transform_webhook_stream(payload, response_control),
                ):
                    pass
            else:
                result = await self._send_payload(payload, allow_tool_only=True)
                response_control.merge(extract_response_control(result))
    ...
    async def _transform_webhook_stream(
        ...
        async for chunk_data in self._send_payload_streaming(payload):
            ...
            response_control.merge(extract_response_control(chunk_data))
            if chunk_data.get("type") in ("item", "final", "status") and "content" in chunk_data:
